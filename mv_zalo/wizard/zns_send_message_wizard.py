...
class ZnsSendMessageWizard(models.TransientModel):
    ...
    def _get_zns_history(self):
        return self.env["zns.history"].search([("msg_id", "=", self.msg_id)], limit=1)

    @api.model
    def _zns_message_template(self, template):
        if template:
            return template
        else:
            return (
    ...
    def process_data(self, data, config_id):
        if self.use_type in ["sale.order", "stock.picking"]:
            sent_time = (
                get_datetime(data.get("sent_time", False))
                if data.get("sent_time", False)
                else ""
