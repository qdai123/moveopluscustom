...
class HrContract(models.Model):
    ...
    def onchange_level(self):
        ...
        if self.level != 0 and self.category_id:
            salary_ids = self.env['basic.salary'].search([("category_id", "=", self.category_id.id)])
            if len(salary_ids) < 1:
                raise ValidationError(_('Config basic salary for %s') % self.job_id)
            elif len(salary_ids) > 1:
                raise ValidationError(_('More config basic salary for %s') % self.job_id)
            else:
                salary_id = salary_ids[0]
                line_ids = salary_id.line_ids.filtered(lambda x: x.level == self.level and x.date <= fields.Date.today()).sorted(lambda l: l.date)
                if len(line_ids) < 1:
                    raise ValidationError(_('No found config basic salary for level %s and job %s') % (self.level, self.job_id.name))
                else:
